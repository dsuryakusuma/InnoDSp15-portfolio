...
(function($){
	...
	MY_APP.contains = function(value1, value2){
		if(value1 === undefined || value2 === undefined){
			return false;
		}
		return value1.toLowerCase().indexOf(value2.toLowerCase()) != -1;
	}
	MY_APP.getDurationInYears = function(greater, startDate, endDate){
	...
	MY_APP.replaceParameters = function(string, values){
	...
	MY_APP.launchTimeline = function(dataUrl, lang){
	...
	$(document).ready(function() {
		...
		$('#ascensorBuilding').ascensor({
			...
			childType: 'section',
			ascensorFloorName: ['home', 'brian', 'catherine', 'eric', 'jacqueline', 'joy', 'monica', 'ziqi'],
			time: 1000,
			windowsOn: 0,
			direction: "chocolate",
			directionOnSize: {type:'width', size:minWidth, direction:"x"},
			ascensorMap: [[0,0],[0,1],[1,0],[1,1],[2,0],[2,1],[3,0],[3,1]],
